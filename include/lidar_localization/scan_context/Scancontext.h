...
{
public:
    friend class boost::serialization::access;   //序列化
    ...
    void init_color();
    cv::Mat get_color();
//    void saveScancontext();
    std::pair<int, float> relocalization(pcl::PointCloud<SCPointType> & _scan_down);
public:
    // hyper parameters ()
    const double LIDAR_HEIGHT = 3; // lidar height : add this for simply directly using lidar scan in the lidar local coord (not robot base coord) / if you use robot-coord-transformed lidar scans, just set this as 0.
