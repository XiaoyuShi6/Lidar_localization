...
bool DataPretreatFlow::Run() {
    if (!ReadData())                 //根据当前帧点云信息来找其对应的IMU   sensor_inited
        return false;

    if (!InitCalibration())          //IMU和Lidar之间的外参calibration_received
        return false;

    if (!InitGNSS())                 //gnss_inited
        return false;

    while(HasData()) {               //各个data_buff是否是空的 buff中存的是跟点云对齐的数据
        if (!ValidData())            //判断数据能不能用
            continue;
...
bool DataPretreatFlow::ReadData() {
    cloud_sub_ptr_->ParseData(cloud_data_buff_);//将收到的点云信息存到cloud_data_buff_

    static std::deque<IMUData> unsynced_imu_;
    static std::deque<VelocityData> unsynced_velocity_;
    static std::deque<GNSSData> unsynced_gnss_;

    imu_sub_ptr_->ParseData(unsynced_imu_);             //取出来未同步的数据
    ...
    bool valid_imu = IMUData::SyncData(unsynced_imu_, imu_data_buff_, cloud_time);//找出与点云对应的一帧IMU
    ...
    if (!sensor_inited) {
        if (!valid_imu || !valid_velocity || !valid_gnss) {
            cloud_data_buff_.pop_front();       //如果找不到对应的，就把该帧点云删掉
            return false;
        }
        sensor_inited = true;
    }
...
bool DataPretreatFlow::TransformData_data() {
    ...
    current_velocity_data_.TransformCoordinate(lidar_to_imu_);            //把速度转换到雷达坐标系，后续这个速度的来源要换成IMU计算
