...
//    pcl::removeNaNFromPointCloud(*local_map_ptr_,*local_map_ptr_,indices);
...
//
//    pcl::PointCloud<pcl::PointXYZ>::Ptr removePoint(new pcl::PointCloud<pcl::PointXYZ>);
//    for (CloudData::CLOUD::iterator i= cloud_data.cloud_ptr->begin(); i !=cloud_data.cloud_ptr->end(); ++i) {
//        if (i->z >-100 && i->z<100)
//        {
//            removePoint->push_back(*i);
//        }
//    }
//    *(cloud_data.cloud_ptr)=*removePoint;
    ...
    std::cout<<"last pose:"<<last_pose<<std::endl<<std::endl;
    std::cout<<"cloud_pose:"<<cloud_pose<<std::endl<<std::endl;
    // 更新相邻两帧的相对运动

    step_pose = last_pose.inverse() * cloud_pose;
    std::cout<<"step_pose:"<<step_pose<<std::endl<<std::endl;
    std::cout<<"last_pose.inverse():"<<last_pose.inverse()<<std::endl<<std::endl;
    predict_pose = cloud_pose * step_pose;
    last_pose = cloud_pose;
    std::cout<<"predict_pose"<<predict_pose<<std::endl<<std::endl;
    // 匹配之后判断是否需要更新局部地图
    std::vector<float> edge = box_filter_ptr_->GetEdge();
    for (int i = 0; i < 3; i++) {
