...
std::string WORK_SPACE_PATH=ros::package::getPath("lidar_localization");//暂时先把路径放在这
...
bool Matching::InitGlobalMap() {
    ...
    std::cout << "filtered global map size:" << global_map_ptr_->points.size();
...
bool Matching::Update(const CloudData& cloud_data, Eigen::Matrix4f& cloud_pose) {
    ...
    static Eigen::Matrix4f step_pose = Eigen::Matrix4f::Identity();  //static全局变量只初始化一次
