...
// namespace SC2
// {
//void SCManager::saveScancontext() {
//
//    std::ofstream file("/home/sxy/sc_ws/src/scan_context/test_data/scoutput.bin");
//    boost::archive::binary_oarchive out(file);
//    std::vector<Eigen::MatrixXd> test=polarcontexts_;
////    std::vector<Eigen::MatrixXd> sc_vec(test,test+test.size());
//
//    out & BOOST_SERIALIZATION_NVP(test);
//
//}
...
cv::Mat SCManager::get_color(){
    //ISCDescriptor isc = isc_arr.back();
    cv::Mat isc_color = cv::Mat::zeros(cv::Size(PC_NUM_SECTOR,PC_NUM_RING), CV_8UC3);
    for (int i = 0;i < scdescriptor.rows;i++) {
        for (int j = 0;j < scdescriptor.cols;j++) {
//            isc_color.at<cv::Vec3b>(i, j) = color_projection[scdescriptor.at<unsigned char>(i,j)];
            isc_color.at<cv::Vec3b>(i,j)[0]=255*(scdescriptor.at<u_char>(i,j));
            isc_color.at<cv::Vec3b>(i,j)[1]=255*(scdescriptor.at<u_char>(i,j)*0.4);
            isc_color.at<cv::Vec3b>(i,j)[2]=255*(scdescriptor.at<u_char>(i,j)*(-0.5)+1);     //b
        }
    }
    return isc_color;
}
...
MatrixXd SCManager::makeScancontext( pcl::PointCloud<SCPointType> & _scan_down )
{
    ...
    cv::eigen2cv(desc,scdescriptor);
//    cout<<"原始"<<scdescriptor<<endl<<endl<<endl;
//
    cv::normalize(scdescriptor,scdescriptor,0,1,cv::NORM_MINMAX);
//    cout<<"归一化后"<<scdescriptor<<endl;
...
void SCManager::makeAndSaveScancontextAndKeys( pcl::PointCloud<SCPointType> & _scan_down )
{
    ...
    Eigen::MatrixXd ringkey = makeRingkeyFromScancontext( sc );        //每一行求均值
    Eigen::MatrixXd sectorkey = makeSectorkeyFromScancontext( sc );      //每列求均值
    std::vector<float> polarcontext_invkey_vec = eig2stdvec( ringkey );     //ringkey转换成向量
...
std::pair<int, float> SCManager::relocalization(pcl::PointCloud<SCPointType> & input)
{
    int loop_id{-1};

    //生成sc
    Eigen::MatrixXd sc = makeScancontext(input); // v1
    Eigen::MatrixXd ringkey = makeRingkeyFromScancontext( sc );        //每一行求均值
    Eigen::MatrixXd sectorkey = makeSectorkeyFromScancontext( sc );      //每列求均值
    std::vector<float> polarcontext_invkey_vec = eig2stdvec( ringkey );     //ringkey转换成向量
    ...
    if(polarcontext_invkeys_mat_.size()<59)
    {
        cout<<"ERROR：SC 库中的sc数量太少"<<endl;
        std::pair<int, float> result {loop_id, 0.0};
        return result; // Early return
    }
...
//        cout << "[Loop found] Nearest distance: " << min_dist << " btn " << polarcontexts_.size()-1 << " and " << nn_idx << "." << endl;
//        cout << "[Loop found] yaw diff: " << nn_align * PC_UNIT_SECTORANGLE << " deg." << endl;
          cout << "[Loop found] Nearest distance: " << min_dist << " btn " << "current scan" << " and " << nn_idx << "." << endl;
          cout << "[Loop found] yaw diff: " << nn_align * PC_UNIT_SECTORANGLE << " deg." << endl;
    }
    else
    {
