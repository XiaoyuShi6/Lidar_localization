...
bool IMUData::SyncData(std::deque<IMUData>& UnsyncedData, std::deque<IMUData>& SyncedData, double sync_time) {
    ...
    while (UnsyncedData.size() >= 2) {
        if (UnsyncedData.front().time > sync_time)  //第一个IMU都比点云时间大，肯定找不到
            return false;
        if (UnsyncedData.at(1).time < sync_time) {
            UnsyncedData.pop_front();              //如果第二个IMU在点云前，则删掉第一个，把第二个当成第一个
            continue;
        }
        if (sync_time - UnsyncedData.front().time > 0.2) {
            UnsyncedData.pop_front();              //如果点云时间比第一个IMU远太多也不对
            return false;
        }
        
        if (UnsyncedData.at(1).time - sync_time > 0.2) {
//            UnsyncedData.pop_front();              //同样第二个IMU也不能离点云太远
            return false;                            //后两个判断由break改成了return false
        }
        break;
    }
